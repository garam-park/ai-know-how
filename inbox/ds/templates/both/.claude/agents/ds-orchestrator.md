...
```
Phase 1: Discovery
  ├─ Brownfield? → Agent 도구로 .claude/agents/auditor.md 호출 (현황 감사)
  └─ Agent 도구로 .claude/agents/planner.md 호출 (Charter, MVP 범위)
  └─ Gate 1 승인

Phase 2: Foundation
  ├─ Agent 도구로 .claude/agents/token-engineer.md 호출 (토큰 3계층)
  ├─ Agent 도구로 .claude/agents/brand-designer.md 호출 (브랜드 가이드라인)
  └─ Gate 2 승인

Phase 3: Build
  ├─ Agent 도구로 .claude/agents/ui-architect.md 호출 (Atomic Design Spec)
  ├─ Agent 도구로 .claude/agents/a11y-engineer.md 호출 (접근성·반응형)
  ├─ Agent 도구로 .claude/agents/visual-archivist.md 호출 (시각 Spec — Storybook 카탈로그 + Penpot On-Demand)
  └─ Gate 3 승인

Phase 4: Ship
  ├─ Agent 도구로 .claude/agents/component-developer.md 호출 (코드 구현)
  ├─ Agent 도구로 .claude/agents/doc-engineer.md 호출 (문서화)
  ├─ Agent 도구로 .claude/agents/release-engineer.md 호출 (배포·QA)
  └─ Gate 4 승인

Phase 5: Evolve
  └─ Agent 도구로 .claude/agents/governance-manager.md 호출 (거버넌스)
  └─ Gate 5 승인
```
